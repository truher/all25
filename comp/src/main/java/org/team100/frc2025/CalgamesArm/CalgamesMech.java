...
public class CalgamesMech extends SubsystemBase {
    private final RotaryMechanism m_armMech;
    ...
    public CalgamesMech(double armLength, int armMotorID, double wristLength, int wristMotorID, LoggerFactory parent,
            int canID) {
        ...
        // configuration, you'd have to do something with the sensors." - joel in
        // mech.java
        m_config = new Config(0.5, 0, 0); // grabbed this from mech.java, unclear what it does, i thought that config
                                          // was for forward

        // all the below is stuff for the arm driving motor, taken from 2025 onseason
        // climber code. not sure what most of it does
        Falcon6Motor armMotor = new Falcon6Motor(log, canID, MotorPhase.REVERSE, 50, 50,
                PIDConstants.makePositionPID(1),
                Feedforward100.makeArmPivot()); // not sure what this does, taken from climber class because arm is a
                                                // falcon
        double inputOffset = 0; // 0 for now, not known real value. this is just the 0 for the encoder, needs to
                                // be calibrated
    ...
    private void addCartesian(double x, double y, double r) {
        Transform2d t = new Transform2d(x, y, new Rotation2d(r)); // how much we want to change our goal point by (eg
        ...
        // finds our new goal point by adding the transformations, those from the first
        // line (eg, 10+5)
