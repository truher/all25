 ...
 * limits.
 * 
 * Mar 6 2025, the need for different acceleration and deceleration rates is
 * apparent, so the "Scalene" (unequal side) version is born.
 * 
 * The Scalene version splits the meaning of "I+" (and I-, G+, and G- similarly)
 * into two: the "accel" limit is used when moving in a positive direction
 * (speeding up), and the "decel" limit is used when moving in a negative
 * direction (slowing down).
 */
public class ScaleneTrapezoidProfile100 implements Profile100 {
    ...
    public ScaleneTrapezoidProfile100(double maxVel, double maxAccel, double maxDecel, double tolerance) {
        m_maxVelocity = maxVel;
        m_maxAcceleration = maxAccel;
        m_maxDeceleration = maxDecel;
    ...
    double c_minus(Model100 s) {
        if (s.v() <= 0) {
            // the path is accelerating (to the left) to reach s
            return s.x() - Math.pow(s.v(), 2) / (-2.0 * m_maxAcceleration);
        }
        // the path is decelerating to reach the intercept
        return s.x() - Math.pow(s.v(), 2) / (-2.0 * m_maxDeceleration);
    }

    /** Intercept of positive-acceleration path intersecting s */
    double c_plus(Model100 s) {
        if (s.v() >= 0) {
            // the path is accelerating to reach s
            return s.x() - Math.pow(s.v(), 2) / (2.0 * m_maxAcceleration);
        }
        // the path is decelerating to reach the intercept
        return s.x() - Math.pow(s.v(), 2) / (2.0 * m_maxDeceleration);
