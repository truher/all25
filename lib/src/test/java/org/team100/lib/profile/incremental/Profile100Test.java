...
import org.team100.lib.profile.incremental.Profile100.ProfileMaker;
import org.team100.lib.state.Control100;
import org.team100.lib.state.Model100;
import org.team100.lib.util.Util;

public class Profile100Test {
    ...
    void testETA() {
        ...
        double s = 1.0;
        ProfileMaker maker = (ss) -> new TrapezoidProfile100(1, 1, 0.01);
        double diff = Profile100.getEtaDiff(0.02, initial, goal, expectedEta, s, maker);
        // eta is indeed 2, it's a triangle path
        assertEquals(0, diff, 0.001);
        // same answer at 10x the step size
        double coarser = Profile100.getEtaDiff(0.2, initial, goal, expectedEta, s, maker);
        assertEquals(0, coarser, 0.001);

        int N = 1000000;
        double t0 = Takt.actual();
        for (int ii = 0; ii < N; ++ii) {
            Profile100.getEtaDiff(0.2, initial, goal, expectedEta, s, maker);
        }
    ...
    void testETA2() {
        Control100 initial = new Control100();
        Model100 goal = new Model100(1, 0);
        Profile100 p = new TrapezoidProfile100(1, 1, 0.01);
    ...
    void testETA2Exponential() {
        Control100 initial = new Control100();
        Model100 goal = new Model100(1, 0);
        Profile100 p = new ExponentialProfileWPI(1, 1);
        double eta = p.simulateForETA(0.02, initial, goal);
