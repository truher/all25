...
/**
 * Arm mechanism with two joints used in 2023.
 */
public class ArmSubsystem23 extends SubsystemBase implements Glassy {
    ...
    private ArmAngles23 m_previousPosition;
    ...
    ArmSubsystem23(
        ...
        Optional<ArmAngles23> position = getPosition();
        if (position.isPresent())
            m_previousPosition = position.get();
    }

    /** Arm angles (radians), 0 up, positive forward. */
    public Optional<ArmAngles23> getPosition() {
        ...
        final ArmAngles23 position = new ArmAngles23(
    ...
    public Optional<ArmAngles23> getVelocity() {
        Optional<ArmAngles23> position = getPosition();
        if (position.isEmpty())
            return Optional.empty();
        ...
        m_previousPosition = position.get();
        final ArmAngles23 velocity = new ArmAngles23(th1 * 50, th2 * 50);
