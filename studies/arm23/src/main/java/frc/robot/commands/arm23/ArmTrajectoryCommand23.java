...
import org.team100.lib.motion.arm23.ArmTrajectories23;
 ...
 */
public class ArmTrajectoryCommand23 extends Command implements Glassy {
    ...
    private final ArmSubsystem23 m_armSubsystem;
    private final ArmKinematics23 m_armKinematicsM;
    private final Translation2d m_goal;

    private final ArmAngles23 m_goalAngles;
    ...
    private final ArmTrajectories23 m_trajectories;
    ...
    public ArmTrajectoryCommand23(
            LoggerFactory parent,
            ArmSubsystem23 armSubSystem,
            ArmKinematics23 armKinematicsM,
            Translation2d goal) {
        ...
        m_trajectories = new ArmTrajectories23(kConf);
    ...
    public void initialize() {
        m_timer.reset();
        Optional<ArmAngles23> position = m_armSubsystem.getPosition();
        if (position.isEmpty())
            return;
    ...
    public void execute() {
        ...
        Optional<ArmAngles23> measurement = m_armSubsystem.getPosition();
        if (measurement.isEmpty())
            return;
        Optional<ArmAngles23> velocityMeasurement = m_armSubsystem.getVelocity();
        if (velocityMeasurement.isEmpty())
            return;

        // position reference
        final ArmAngles23 r = getThetaPosReference(desiredState);
        ...
        // velocity reference
        final ArmAngles23 rdot = getThetaVelReference(desiredState, r);
