...
public class TankFactory {
    ...
    private static TankDrive realRobot(LoggerFactory fieldLogger, LoggerFactory parent, int supplyLimit) {
        ...
        PIDConstants pid = new PIDConstants(1, 1);
        NeoCANSparkMotor right = new NeoCANSparkMotor(
                log.name("right"), new CanId(5), NeutralMode.BRAKE,
                MotorPhase.FORWARD, supplyLimit, ff, pid);

        NeoCANSparkMotor left = new NeoCANSparkMotor(
                log.name("left"), new CanId(6), NeutralMode.BRAKE,
                MotorPhase.REVERSE, supplyLimit, ff, pid);
